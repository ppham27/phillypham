             ...
             new LocalStrategy({
               ...
               passReqToCallback: true
                  ...
                  sweetCaptcha.api('check', {sckey: user.sckey, scvalue: user.scvalue, scvalue2: user.scvalue2}, function(err, valid) {
                    ...
                    // don't allow blank photourls
                    user.photoUrl = user.photoUrl || db.User.tableAttributes.photoUrl.defaultValue; 
                    user.userGroupId = db.ApplicationSettings.defaultUserGroupId;
                    if (typeof user.facebookUsername === 'string') {
                      user.facebookUsername = user.facebookUsername || null;
                    }
                    return db.User.findOrCreate({where: {email: user.email}, defaults: user})
                           .spread(function(newUser, created) {
                             if (created) {
                               ...
                               emailVerifier.verify(newUser)
                               .catch(function(err) {                                        
                                        // assume success and silently fail
                                        console.error(err);
                                      });
                               if (session) delete req.session.temporaryUser; // we don't need this anymore
...
passport.use('emailVerify',
             new HashStrategy(
               function(hash, done) {                  
                 redisClient.get('emailVerifyToken:' + hash, function(err, email) {
                   if (email === null)                     
                     return done(null, false, 
                                 'Your authentication token has expired. Please request a new confirmation email in user settings');
                   db.User.findOne({where: {email: email}})
                   .then(function(user) {
                     user.emailVerified = true;
                     emailVerifier.deleteToken(email);
                     return user.save();
                   })
                   .then(function(user) {
                     return done(null, user, 'Your email address has been confirmed.');
                   });
                 });
               }
             ));
                  ...
                  return db.User.findOne({where: {facebookId: profile.id}})
                         ...
                         .then(function(user) {
                           ...
                           if (user !== null && user.id && user.facebookId !== profile.id) {
                             ...
                             // old user, if photo needs new token
                             if (/fbcdn/.test(user.photoUrl)) user.photoUrl = profile.photos[0].value;
                             return user.save();
                           }
                         })
                         .then(function(user) {

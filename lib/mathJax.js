...
var preview = null;
var inline = '$';
var blocks, start, end, last, braces;
var HUB, InputJax;
...
//
function processMath(i, j, unescape, math) {
...
function removeMath(state) {
  var text = state.text;
  start = end = last = null;
  var math = state.mathJaxMath;
  var unescape;
  if(/`/.test(text)) {
  ...
  for(var i = 1, m = blocks.length; i < m; i += 2) {
	...
	if('@' === block.charAt(0)) {
	  ...
	  if(end == inline && block.charAt(0) == '\n') {
		if(last) {
		  i = last;
		  processMath(start, i, unescape, math);
		}
		start = end = last = null;
		braces = 0;
	  }
  ...
  last && processMath(start, last, unescape, math);
  state.text = unescape(blocks.join(''));
  return state;
}
...
function restartMathJax() {
  ...
  HUB.Queue(
    ["resetEquationNumbers", InputJax.TeX],
    ['Typeset', HUB, preview]
  );
}
